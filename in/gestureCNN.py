...
def loadCNN(wf_index):
    ...
    if wf_index >= 0:
        ...
        # fname = WeightFileName[int(wf_index)] # ori_4015imgs_weights.hdf5
...
def guessGesture(model, img):
    ...
    if prob > 70.0:
        ...
        return output.index(guess), guess+" "+str(round(prob, 3))

    else:
        return 1, "NOTHING" # nothing idx = 1
...
# only called in train model
def initializers():
    imlist = modlistdir(path2) # image file list
...
# Never Called : must be visualize = True
